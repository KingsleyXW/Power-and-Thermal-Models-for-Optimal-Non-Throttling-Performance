...
average_frequency, Total_utilization, power, average_CPU_T,frequency_CPU_5,frequency_CPU_6,frequency_CPU_7, CPU_0_5_average_U, CPU_6_average_U, CPU_7_average_U, CPU_0_2_5, WEDC,time_stamp, downlink, uplink = data_prepocessing(original_data)
...
if SETTING_CONDITION == 0:
    ...
    X_train = np.array([Total_utilization.reshape(N_train,-1),
                        CPU_6_average_U.reshape(N_train,-1), CPU_7_average_U.reshape(N_train,-1), average_CPU_T.reshape(N_train,-1)])
    # X_train = np.array([Total_utilization.reshape(N_train,-1),
...
# prepare test data
average_frequency_test, Total_utilization_test, power_test, average_CPU_T_test, frequency_CPU_5_test,frequency_CPU_6_test,frequency_CPU_7_test, CPU_0_5_average_U_test, CPU_6_average_U_test, CPU_7_average_U_test, CPU_0_2_5_test, WEDC_test, time_stamp_test, downlink_test, uplink_test = data_prepocessing(test_data)
...
if SETTING_CONDITION == 0:
    ...
    X_test = np.array([Total_utilization_test.reshape(N_test,-1), 
                        CPU_6_average_U_test.reshape(N_test,-1), CPU_7_average_U_test.reshape(N_test,-1), average_CPU_T_test.reshape(N_test,-1)])
    # X_test = np.array([Total_utilization_test.reshape(N_test,-1), 
...
if SETTING_CONDITION !=5:
    train_y = train_y-base_power_cpu
    train_y[train_y<0] = 0
    # Ordinary Linear Regression
    w=np.zeros((M_train,1))
    w=np.linalg.pinv(Train_X).dot(train_y) # pinv(A)*b
    Predict_y_OLS=Train_X.dot(w)+base_power_cpu
    train_y = train_y+base_power_cpu
    ...
    base_power_iperf = 750
    base_power_5g = 3600- base_power_iperf
    w_cpu = np.loadtxt('./Original.txt')
    w_cpu = w_cpu.reshape(w_cpu.shape[0],-1)

    train_y = train_y-Train_X[:,0:w_cpu.shape[0]].dot(w_cpu)
    # for i in range(N_train):
    #     if downlink[i]>10:
    #         train_y[i] = train_y[i]-base_power_iperf - base_power_5g
    #     else:
    #         train_y[i] = train_y[i]-10000
    #         print(train_y[i])
    train_y[train_y<0] = 0
    # Ordinary Linear Regression
    w_transc=np.zeros((M_train-w_cpu.shape[0],1))
    w_transc=np.linalg.pinv(Train_X[:,w_cpu.shape[0]:]).dot(train_y) # pinv(A)*b
    print(w_transc.shape)
    Predict_y_OLS=Train_X[:,w_cpu.shape[0]:].dot(w_transc)+base_power_iperf
    train_y = train_y+base_power_iperf + Train_X[:,0:w_cpu.shape[0]].dot(w_cpu)
    w = np.concatenate((w_cpu,w_transc),0)
    
    np.savetxt('Final_coefficient.txt', w)
...
if SETTING_CONDITION !=5:
    test_y = test_y-base_power_cpu
    Predict_y_OLS_test=Test_X.dot(w)+base_power_cpu
    test_y[test_y<0] = 0
    test_y = test_y+base_power_cpu
elif SETTING_CONDITION ==5:
    base_power_iperf = 750
