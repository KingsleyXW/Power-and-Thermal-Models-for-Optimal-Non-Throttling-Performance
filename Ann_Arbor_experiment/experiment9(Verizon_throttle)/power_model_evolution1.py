...
base_power_5g = 3800 - base_power_iperf
...
Predict_y_OLS_test[Predict_y_OLS_test<5500] = 0

pred_1 = np.array(Predict_y_OLS_test)


# w = np.array([2.458372933123951043e+02,
# -2.996057955801871913e+02,
# 4.509271016622911930e+02,
# 5.942318217386208801e-01,
# 3.282574987861567539e+00,
# 3.622582392234073723e+01])
...
# Predict_y_OLS_test = Predict_y_OLS_test-500
# Predict_y_OLS_test[Predict_y_OLS_test>5500] = 0
# pred_2 = np.array(Predict_y_OLS_test)

# Predict_y_OLS_test = pred_1 + pred_2
# Predict_y_OLS_test[Predict_y_OLS_test<750] = 750

Predict_y_OLS_test_CPU = Test_X[:,:3].dot(w[:3,0].reshape(3,1))+750-300
Predict_y_OLS_test_tramsceiver = Test_X[:,3:].dot(w[3:,0].reshape(3,1))+3900-750
...
plt.legend(['Transceiver Power ','CPU Power'], loc=8)
...
# plt.plot(time_stamp_test, pred_1, 'r*-', linewidth=1.5)
# plt.plot(time_stamp_test, pred_2, 'r*-', linewidth=1.5)
plt.xlabel("Elapsed Time (s)")
plt.ylabel("Prediction (mW)")
