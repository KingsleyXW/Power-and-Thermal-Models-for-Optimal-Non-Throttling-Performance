...
if SETTING_CONDITION == 0:
    ...
    X_train = np.array([Total_utilization.reshape(N_train,-1),
                        CPU_6_average_U.reshape(N_train,-1), CPU_7_average_U.reshape(N_train,-1), average_CPU_T.reshape(N_train,-1), downlink.reshape(N_train,-1),uplink.reshape(N_train,-1)])
    
    X_train = np.array([Total_utilization.reshape(N_train,-1),
                        CPU_6_average_U.reshape(N_train,-1), CPU_7_average_U.reshape(N_train,-1), downlink.reshape(N_train,-1),uplink.reshape(N_train,-1)])
...
if SETTING_CONDITION == 0:
    ...
    X_test = np.array([Total_utilization_test.reshape(N_test,-1),
     ...
     X_test = np.array([Total_utilization_test.reshape(N_test,-1),
                        CPU_6_average_U_test.reshape(N_test,-1), CPU_7_average_U_test.reshape(N_test,-1), average_CPU_T_test.reshape(N_test,-1),downlink_test.reshape(N_test,-1), uplink_test.reshape(N_test,-1)])
     
     X_test = np.array([Total_utilization_test.reshape(N_test,-1),
                        CPU_6_average_U_test.reshape(N_test,-1), CPU_7_average_U_test.reshape(N_test,-1), downlink_test.reshape(N_test,-1), uplink_test.reshape(N_test,-1)])
...
w = np.array([4.890679722767078488e+02,
5.921009107118445058e+01,
2.181915420795522351e+02,
1.135286385964230593e+00,
1.028548481313721368e+02])
...
if SETTING_CONDITION == 5:
    ...
    for i in range(N_test):
        if downlink_test[i]>5:
            Predict_y_OLS_test[i] = Predict_y_OLS_test[i]+base_power_iperf+base_power_5g
        else:
            Predict_y_OLS_test[i] = Predict_y_OLS_test[i]+base_power_iperf-250
...
cpu_base = 450
iperf_base = 750
cpu_transc_base_diff = iperf_base-cpu_base
Predict_y_OLS_test_CPU = Test_X[:,:3].dot(w[:3,0].reshape(3,1))+cpu_base
Predict_y_OLS_test_tramsceiver = Test_X[:,3:].dot(w[3:,0].reshape(2,1))+3900-750

Predict_y_OLS_test_tramsceiver[Predict_y_OLS_test_tramsceiver<1500] = 0

plt.figure(15)
fig,ax=plt.subplots()
ax.plot(time_stamp_test, Predict_y_OLS_test_tramsceiver/Predict_y_OLS_test_CPU, color="y",linestyle='-.', linewidth=1.5)

ax.set_xlabel("time_stamp (s)")
plt.legend(['Prediction Power Rate'], loc=1)
ax.set_ylabel("Prediction Power Rate(transicever/CPU)",fontsize=14)
plt.ylim(0, 12)
ax2=ax.twinx()
# make a plot with different y-axis using second axis object
ax2.plot(time_stamp_test, Predict_y_OLS_test_tramsceiver ,'ob-', linewidth=1.5)
ax2.plot(time_stamp_test, Predict_y_OLS_test_CPU ,'ro-', linewidth=1.5)
ax2.set_ylabel("Prediction Power",color="red", fontsize=14)
# plt.ylim(20, 50)
plt.title("Power Consumption proportion Prediction ")
plt.legend(['Transceiver Power ','CPU Power'], loc=2)
plt.savefig('trx_CPU_power.png')

# plt.figure(2)
# plt.plot(time_stamp_test, Predict_y_OLS_test_tramsceiver/Predict_y_OLS_test_CPU, 'go-', linewidth=1.5)
# plt.xlabel("Elapsed Time (s)")
# plt.ylabel("Prediction Power Rate (mW)")
# plt.title("Power Portion Comparison")
# plt.legend(['Testing Data', 'Prediction'])
# plt.savefig('Training results.png')
...
index_thr = np.argsort(downlink)
index_usage = np.argsort(Total_utilization)
power_sorted_thr = power[index_thr]
power_sorted_usage = power[index_usage]
downlink_rate_sorted = downlink[index_thr]
Total_utilization_sorted = Total_utilization[index_usage]
Predict_y_OLS_sorted_thr = Predict_y_OLS[index_thr]
Predict_y_OLS_sorted_usage = Predict_y_OLS[index_usage]


plt.figure(12)
plt.plot(downlink_rate_sorted, power_sorted_thr, marker="o",  markersize=5)
plt.plot(downlink_rate_sorted, Predict_y_OLS_sorted_thr, marker="o",  markersize=5)
# plt.plot(downlink_rate_sorted, power_sorted_thr, marker="o",  markersize=5)
...
plt.plot(Total_utilization_sorted, Predict_y_OLS_sorted_usage, marker="o",  markersize=5)
...
ax.scatter3D(Total_utilization[index_usage], downlink[index_usage], Predict_y_OLS[index_usage], c=Predict_y_OLS[index_usage], cmap='hot');
ax.scatter3D(Total_utilization[index_usage], downlink[index_usage], power[index_usage], c=power[index_usage], cmap='Blues');
plt.ylabel("Downlink_throughput",color="blue",fontsize=11)
plt.xlabel("total usage",color="black",fontsize=11)
# plt.zlabel("power",color="blue",fontsize=14)
index_thr_test = np.argsort(downlink_test)
index_usage_test  = np.argsort(Total_utilization_test)
# print(index_thr_test.shape,power.shape)
power_sorted_thr_test  = power_test[index_thr_test]
power_sorted_usage_test  = power_test[index_usage_test ]
downlink_rate_sorted_test  = downlink_test[index_thr_test]
Total_utilization_sorted_test  = Total_utilization_test[index_usage_test]
Predict_y_OLS_sorted_thr_test  = Predict_y_OLS_test[index_thr_test]
Predict_y_OLS_sorted_usage_test  = Predict_y_OLS_test[index_usage_test]


fig = plt.figure()
ax = plt.axes(projection='3d')
ax.scatter3D(Total_utilization_test[index_usage_test], downlink_test[index_usage_test], Predict_y_OLS_test[index_usage_test], c=Predict_y_OLS_test[index_usage_test], cmap='hot');
ax.scatter3D(Total_utilization_test[index_usage_test], downlink_test[index_usage_test], power_test[index_usage_test], c=power_test[index_usage_test], cmap='Blues');
plt.ylabel("Downlink_throughput",color="blue",fontsize=11)
plt.xlabel("total usage",color="black",fontsize=11)
# plt.zlabel("power",color="blue",fontsize=14)
